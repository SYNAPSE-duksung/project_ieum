...
class IEUMDataset(Dataset):
    ...
    def _build_samples(
        ...
        for (audio_filename, segment_id), group in grouped:
            ...
            sample_id = f"{audio_filename}::{segment_id}"
            ...
            segment_start_sec = float(
                group[self.segment_start_column].min()
            )

            segment_end_sec = float(
                group[self.segment_end_column].max()
            )

            duration_seconds = (
                segment_end_sec - segment_start_sec
            )
            ...
            if self.audio_path_column in group.columns:
                ...
                if path_values:
                    stored_audio_path = path_values[0]
        ...
        if self.min_audio_seconds is not None:
            too_short_mask = (
                ...
                < float(self.min_audio_seconds)
            )

        if self.max_audio_seconds is not None:
            too_long_mask = (
                samples["duration_seconds"]
                > float(self.max_audio_seconds)
            )

        keep_mask = ~(too_short_mask | too_long_mask)
        ...
        filter_summary = {
            ...
            "removed_too_short": int(too_short_mask.sum()),
            "removed_too_long": int(too_long_mask.sum()),
            "total_after_filter": int(len(filtered_samples)),
        }

        if filtered_samples.empty:
            raise ValueError(
                "음성 길이 필터 적용 후 남은 데이터가 없습니다."
            )

        return filtered_samples, filter_summary

    def _create_audio_index(self) -> dict[str, Path]:
        """
        audio_root 아래 WAV 파일을 파일명 기준으로 인덱싱한다.

        CSV에 저장된 Colab 경로가 유효하지 않을 때 사용한다.
        """
        ...
        for audio_path in self.audio_root.rglob("*.wav"):
            ...
            if filename in audio_index:
                ...
                audio_index[filename] = audio_path

        if duplicate_paths:
            duplicate_name = next(iter(duplicate_paths))
            example_paths = duplicate_paths[duplicate_name]

            raise ValueError(
                "audio_root 아래 동일한 파일명의 WAV가 "
                "여러 개 존재합니다.\n"
    ...
    def _resolve_audio_path(
        ...
        if stored_audio_path:
            stored_path = Path(stored_audio_path)

            if stored_path.exists():
                return stored_path

        direct_path = self.audio_root / audio_filename

        if direct_path.exists():
            return direct_path

        if self.audio_index is None:
            self.audio_index = self._create_audio_index()

        indexed_path = self.audio_index.get(audio_filename)
    ...
    def _load_audio_segment(
        ...
        frame_offset = int(
            round(start_sec * original_sample_rate)
        )
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        if not self.load_audio:
            sample["stored_audio_path"] = row[
                "stored_audio_path"
            ]
            return sample

        audio_path = self._resolve_audio_path(
            stored_audio_path=row["stored_audio_path"],
            audio_filename=row["audio_filename"],
        )
        ...
        waveform = self._load_audio_segment(
            ...
            end_sec=float(row["segment_end_sec"]),
        )
    ...
    def get_metadata(self) -> pd.DataFrame:
        """샘플 단위 메타데이터 복사본을 반환한다."""
        return self.samples.copy()

    def summary(self) -> dict[str, Any]:
        """Dataset의 기본 통계를 반환한다."""
        duration = self.samples["duration_seconds"]
        ...
        return {
            ...
            "speaker_count": int(
                self.samples["speaker_id"].nunique()
            ),
            "split_counts": (
                self.samples["split"]
                .value_counts()
